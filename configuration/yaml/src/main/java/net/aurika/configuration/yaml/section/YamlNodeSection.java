...
public class YamlNodeSection extends AbstractConfigSection implements ConfigSection, YamlConfigSection {
  ...
  private static final ScalarNode UNKNOWN_PARENT = new ScalarNode(
      Tag.STR, ConfigSection.UNKNOWN_PARENT, ScalarStyle.PLAIN);
  ...
  public YamlNodeSection(@NotNull ConfigSection parent, @NotNull ScalarNode key, @NotNull Node root) {
    super(
        parent, Objects.requireNonNull(key, "Key node must be provided.").getValue(),
        standardTagToLabel(root.getTag())
    );
  ...
  private static String validatePathEnd(ConfigEntry path, ScalarNode key) {
    ...
    if (!Objects.equals(path.getEnd(), key.getValue())) {
      throw new IllegalArgumentException(
          "Last section of path " + path.asString() + " is not contains the scalar node value: " + key.getValue());
    } else {
      return key.getValue();
    }
  }
  ...
  public void setConfigureString(String configure) {
    if (this.root instanceof ScalarNode) {
      this.root = configure == null ? new ScalarNode(this.root.getTag(), "~", ScalarStyle.PLAIN) : new ScalarNode(
          this.root.getTag(), configure, ScalarStyle.DOUBLE_QUOTED);
    }
  }
  ...
  public static YamlNodeSection empty() {
    return new YamlNodeSection(
        new MappingNode(Tag.MAP, new ArrayList<>(), FlowStyle.BLOCK), ConfigEntry.empty(), "", null);
  }

package net.aurika.configuration.yaml.accessor;
...
public class YamlClearlyConfigAccessor implements ClearlyConfigAccessor {
  ...
  public String getString(String... path) {
    return !this.noDefault && !this.isSet(path) ? this.defaultSection.getString(path) : this.currentSection.getString(
        path);
  }
  ...
  public YamlUndefinedPathConfigAccessor get(String... path) {
    YamlUndefinedPathConfigAccessor accessor = new YamlUndefinedPathConfigAccessor(
        this.currentSection, this.defaultSection, new ConfigPath(path));
    if (this.noDefault) {
      accessor.noDefault();
    }
  ...
  public Boolean getBoolean(String... path) {
    return !this.noDefault && !this.isSet(path) ? this.defaultSection.getBoolean(path) : this.currentSection.getBoolean(
        path);
  }

  public Integer getInteger(String... path) {
    return !this.noDefault && !this.isSet(path) ? this.defaultSection.getInteger(path) : this.currentSection.getInteger(
        path);
  }
  ...
  public Double getDouble(String... path) {
    return !this.noDefault && !this.isSet(path) ? this.defaultSection.getDouble(path) : this.currentSection.getDouble(
        path);
  }

  public List<Integer> getIntegerList(String... path) {
    return !this.noDefault && !this.isSet(path) ? this.defaultSection.getIntegerList(
        path) : this.currentSection.getIntegerList(path);
  }

  public List<String> getStringList(String... path) {
    return !this.noDefault && !this.isSet(path) ? this.defaultSection.getStringList(
        path) : this.currentSection.getStringList(path);
  }

  public MathCompiler.Expression getMath(String... path) {
    return !this.noDefault && !this.isSet(path) ? this.defaultSection.get(
        path, NodeInterpreter.MATH) : this.currentSection.get(path, NodeInterpreter.MATH);
  }

  public ConditionCompiler.LogicalOperand getCondition(String... path) {
    return !this.noDefault && !this.isSet(path) ? this.defaultSection.get(
        path, NodeInterpreter.CONDITION) : this.currentSection.get(path, NodeInterpreter.CONDITION);
  }
  ...
  public YamlClearlyConfigAccessor gotoSection(@NotNull String @NotNull [] path) {
    ...
    if (newCurrentSection != null) {
      newClearly = new YamlClearlyConfigAccessor(
          newCurrentSection, this.noDefault ? newCurrentSection : newDefaultSection);
      if (this.noDefault) {

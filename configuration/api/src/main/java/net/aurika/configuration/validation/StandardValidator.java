...
import net.aurika.configuration.validation.ValidationFailure.Severity;
  ...
  public ValidationFailure validate(ValidationContext context) {
    ...
    if (!(section.getBranchState() == ConfigSection.BranchState.END)) {
      return context.fail(new ValidationFailure(
          Severity.ERROR, section,
          "Wrong type, expected '" + this.type.name + "' but got '" + section.getLabel().getValue() + '\''
      ));

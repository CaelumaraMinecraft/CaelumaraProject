...
public class ConfigPath {
  ...
  public ConfigPath removePrefix(String[] other) {
    if (ArrayUtils.startsWith(this.path, other)) {
      ...
      throw new IllegalArgumentException("This path is not starts with other String array: " + Arrays.toString(
          this.path) + ", other: " + Arrays.toString(other));
    }
  }
...
//    }
          ...
          do {
            if (!iterator.hasNext()) {
              throw new IllegalArgumentException(
                  "Cannot find replacement variable for '" + var5 + "' -> " + replacementVariables);
            }
  ...
  public static ConfigPath fromEnum(String name) {
    ...
    if (indexes.length % 2 != 0) {
      throw new IllegalArgumentException(
          "Unclosed variable surround, found total" + indexes.length + " '$' of enum name: " + name);
    }

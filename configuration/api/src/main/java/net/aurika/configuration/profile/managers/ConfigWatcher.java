...
import net.aurika.auspice.scheduler.DelayedTask;
import net.aurika.auspice.utils.AuspiceLogger;
import net.aurika.auspice.utils.cache.caffeine.CacheHandler;
import net.aurika.auspice.utils.cache.caffeine.ExpirableSet;
import net.aurika.auspice.utils.cache.caffeine.ExpirationStrategy;
import net.aurika.configuration.adapter.YamlContainer;
...
public class ConfigWatcher {
  ...
  private static final ExpirableSet<String> a = new ExpirableSet<>(
      ExpirationStrategy.expireAfterCreate(Duration.ofSeconds(3L)));
  private static final Cache<String, DelayedTask> b = CacheHandler.newBuilder().expireAfterWrite(
  ...
  public static WatchKey register(Path var0, BiConsumer<Path, WatchEvent.Kind<?>> var1) {
    try {
      WatchKey var2 = var0.register(
          WATCH_SERVICE, StandardWatchEventKinds.ENTRY_CREATE, StandardWatchEventKinds.ENTRY_DELETE,
  ...
  public static void registerGUIWatchers(SupportedLanguage var0) {
    ...
    if (Files.exists(var1 = var0.getGUIFolder(), new LinkOption[0])) {
      ...
      try {
        Files.walkFileTree(
            var1, new SimpleFileVisitor<Path>() {
            }
        );
        ...
        while (true) {
          while (true) {
            ...
            if (var8 == null && (var7 || var9 > 0L)) {
              ...
              b.put(
                  var6, Auspice.taskScheduler().async().delayed(
  ...
  static void a(Path var0, WatchEvent.Kind<?> var1) {
    Path var3;
    String var2 = (var2 = (var3 = d.getDataFolder().toPath()).relativize(var3.resolve("Structures")).relativize(
        var0).toString()).substring(0, var2.length() - 4);
    StructureStyle var4 = StructureRegistry.get().getStyle(var2);
    MessageHandler.sendConsolePluginMessage(
        "&2Detected changes for structure&8: &9" + var2 + (var4 == null ? " &4which is not a registered structure format, ignoring." : ""));
    if (var4 != null) {
  ...
  static void b(Path var0, WatchEvent.Kind<?> var1) {
    Path var3;
    String var2 = (var2 = (var3 = d.getDataFolder().toPath()).relativize(var3.resolve("Turrets")).relativize(
        var0).toString()).substring(0, var2.length() - 4);
    TurretStyle var4 = TurretRegistry.get().getStyle(var2);
    MessageHandler.sendConsolePluginMessage(
        "&2Detected changes for turret&8: &9" + var2 + (var4 == null ? " &4which is not a registered turret format, ignoring." : ""));
    if (var4 != null) {
  ...
  static void c(Path var0, WatchEvent.Kind<?> var1) {
    String var2;
    SupportedLanguage var3;
    if ((var3 = LanguageManager.getLanguageOrWarn(
        var2 = (var2 = var0.getFileName().toString()).substring(0, var2.length() - 4))) != null) {
      if (var3.isInstalled()) {
  ...
  public static void handleNormalConfigs(Path var0, WatchEvent.Kind<?> var1) {
    String var2;
    String var3 = (var2 = var0.toString()).toLowerCase(Locale.ENGLISH).replace('\\', '/').substring(
        0, var2.length() - 4);
    FileWatcher var4;

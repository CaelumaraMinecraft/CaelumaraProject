...
public interface DyanasisFunctionContainer<F extends DyanasisFunction> {
     ...
     * @return the function
     */
    @Nullable F getFunction(@NotNull DyanasisFunctionKey key);
     ...
     */
    @Unmodifiable
    @NotNull Map<DyanasisFunctionKey, ? extends F> allFunctions();
    ...
    default @NotNull Map<Integer, DyanasisFunction> getSameNameFunction(@NotNull String name) {
        ...
        Map<DyanasisFunctionKey, ? extends F> fns = this.allFunctions();
        for (Map.Entry<DyanasisFunctionKey, ? extends F> entry : fns.entrySet()) {

...
public final class PluginChannels {
  ...
  static {
    if (Reflect.classExists("net.minecraft.resources.MinecraftKey") || Reflect.classExists(
  ...
  public enum DefaultChannel {
    ...
    DefaultChannel() {
      try {
        this.minecraftKey = (MinecraftKey) MINECRAFTKEY.invoke(
            "minecraft", this.name().replace('$', '/').toLowerCase(Locale.ENGLISH));
      } catch (Throwable e) {
  ...
  public static void clearBlockMarkers(@NotNull final Player player, @NotNull Collection<BlockMarkerPluginChannel> markers) {
    ...
    for (BlockMarkerPluginChannel marker : markers) {
      ...
      for (Map.Entry<Grid3Pos.AbstractGrid3D, BlockMarker> block : marker.getMarkers().entrySet()) {
        Grid3Pos.AbstractGrid3D loc = block.getKey();
        Object packet = RecordAccessor.createCustomPayload(
            loc.gridX(), loc.gridY(), loc.gridZ(),
  ...
  //
  // public static void sendBlockMarker(@NotNull final Player player, @NotNull BlockVector3 location, Color chat, Duration duration) {
  ...
  //
  //     // Encoded ARGB chat
  //     int argb = (0xFF & chat.getAlpha()) << 24
  //             | (0xFF & chat.getRed()) << 16
  //             | (0xFF & chat.getGreen()) << 8
  //             | (0xFF & chat.getBlue());

...
public final class MathCompiler {
  ...
  public static final @NotNull Expression DEFAULT_VALUE = new ConstantExpr(0.0, ConstantExprType.NUMBER);
  ...
  public static Expression compile(@Nullable String str) throws NumberFormatException, ArithmeticException {
    return str == null || str.isEmpty() ? MathCompiler.DEFAULT_VALUE : compile_expr(
        (new MathCompiler(str, 0, str.length(), false, new LinkedList<>())).a()).withOriginalString(str);
  }
      ...
      do {
        ...
        if (var1 != ' ') {
          ...
          if ('0' <= var1 && var1 < ':') {
            ...
            for (indexCh = this.string.charAt(
                var4); ('0' <= indexCh && indexCh < ':') || indexCh == 'x' || indexCh == 'e' || indexCh == 'E' || indexCh == '-' || indexCh == '.'; indexCh = compiler.string.charAt(
                var4)) {
              if (indexCh == '-') {
          ...
          if (var33 == null) {
            ...
            if (var37 == null) {
              throw e(
                  this.index,
                  "Unrecognized character '" + var1 + "' (" + var1 + ") outside of variable/placeholder interpolation",
                  null
              );
            }
...
//                            }
  ...
  private Expression b() {
    if ((this.index >= this.length || !this.d) && !this.environments.isEmpty()) {
      ...
      throw this.e(
          this.environments.getLast().getIndex(), "Unclosed parentheses" + (var9.element ? " and functions" : ""),
          var11
      );
      ...
      if (this.mathObjects.size() == 1) {
        ...
        if ((var2 = this.mathObjects.getLast()) instanceof Operator) {
          ...
          throw e(
              this.length - 1, "Blank operand on right hand side of " + ((Operator) var2).getSymbol() + var10, null);
        } else {
    ...
    if (function == null) {
      ...
      if (function.getArgCount() < 0) {
        ...
        if (expressions.size() < var10) {
          throw this.e(
              var5,
              "Too few arguments for function '" + funcStr + "', expected at least: " + var10 + ", got: " + expressions.size(),
              pointerToName(var5, funcStr)
          );
        }
      } else {
        if (expressions.size() < function.getArgCount()) {
          throw this.e(
              var5,
              "Too few arguments for function '" + funcStr + "', expected: " + function.getArgCount() + ", got: " + expressions.size(),
              pointerToName(var5, funcStr)
          );
        }
        ...
        if (expressions.size() > function.getArgCount()) {
          throw this.e(
              var5,
              "Too many arguments for function '" + funcStr + "', expected: " + function.getArgCount() + ", got: " + expressions.size(),
              pointerToName(var5, funcStr)
          );
        }
      }
  ...
  public static String findFunction(String name) {
    ...
    Optional<String> object2 = MathCompiler.getFunctions().keySet().stream().filter(
        funcName -> funcName.equals(lowerCaseName)).findFirst();  //TODO class 1 ? 2
  ...
  private static void default$registerFunctions() {
    ...
    registerFunction(
        "random", false, args -> ThreadLocalRandom.current().nextDouble(args.next(), args.next() + 1.0), 2);
    registerFunction(
        "randInt", false, args -> ThreadLocalRandom.current().nextInt((int) args.next(), (int) args.next() + 1), 2);
  ...
  private static void default$registerOperators() {
    ...
    registerOperator(new Operator('<', 8, (left, right) -> (double) ((long) left << (int) ((long) right))));      //左移
    registerOperator(
        new Operator('$', 8, (left, right) -> (double) ((long) left >>> (int) ((long) right))));     //无符号右移
    ...
    if (expr instanceof BiOperation biOp) {
      ...
      if (leftExpr instanceof ConstantExpr && rightExpr instanceof ConstantExpr) {
        return new ConstantExpr(
            biOp.getOperator().getFunction().apply(
                ((ConstantExpr) leftExpr).getValue$core(),
                ((ConstantExpr) rightExpr).getValue$core()
            ), ConstantExprType.OPTIMIZED
        );
      }
      ...
      if (isConstantExpr) {
        return new ConstantExpr(
            funExpr.getHandler().getFunction().apply(new FnArgs(funExpr, MathCompiler.wrong)),
            ConstantExprType.OPTIMIZED
        );
      }
  ...
  public abstract static class Expression implements MathExpression {
    ...
    public final <T extends Expression> boolean contains(@NotNull Class<T> type, @NotNull Predicate<T> predicate) {
      ...
      if (this instanceof BiOperation) {
        return ((BiOperation) this).getLeft().contains(type, predicate) || ((BiOperation) this).getRight().contains(
            type, predicate);
  ...
  public static final class FunctionExpr extends Expression {
    ...
    public FunctionExpr(@NotNull String name, @NotNull MathFunction handler, @NotNull Expression[] args) {
      ...
      Validate.Arg.notNull(args, "args");
      this.name = name;

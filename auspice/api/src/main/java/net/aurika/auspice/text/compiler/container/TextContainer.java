  ...
  static TextContainer parse(UndefinedPathConfigAccessor undefinedPathConfigAccessor) {
    ...
    if (accessor != null) {
      ...
      for (String keyStr : accessor.getKeys()) {
        ...
        if (msgStr == null) {
          AuspiceLogger.warn(
              "Warn while parsing at path: '" + section.getPath().asString() + "', there is nothing!");  //TODO
        } else {

          TextObject var7 = TextCompiler.compile(
              msgStr, TextCompiler.defaultSettingsWithErrorHandler((TextCompiler messageCompiler) -> {
...
//                        AuspiceLogger.warn("While parsing '" + keyedConfigAccessor.getOptionPath() + "' macro, line " + startMark.getLine() + ":\n" + messageCompiler.joinExceptions());
                ...
                AuspiceLogger.warn(
                    "While parsing '" + undefinedPathConfigAccessor.getOptionPath() + "' macro:\n" + messageCompiler.joinExceptions());
              })
          );
      ...
      parsed = new SimpleTextContainer(TextCompiler.compile(
          undefinedPathConfigAccessor.getString(), TextCompiler.defaultSettingsWithErrorHandler((messageCompiler) -> {

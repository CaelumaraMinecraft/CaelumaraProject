...
public class CommandContext extends SimpleMessenger {
  ...
  public CommandContext(AuspiceLoader var1, KingdomsCommand command, CommandSender messageReceiver, String[] args) {
    super(messageReceiver, MessageContext.messageContext().withContext(messageReceiver));
    this.loggingLevel = CommandLoggingLevel.ALL;
    this.c = false;
    super.messageSettings.lang(this.isPlayer() ? this.getAuspicePlayer().diversity() : Diversity.globalDefault());
    this.plugin = var1;
  ...
  public boolean hasPermission() {
    return !(super.sender instanceof Player) || this.command.hasPermission(
        super.sender) || AuspicePlayer.getAuspicePlayer((Player) super.sender).isAdmin();
  }
  ...
  public void requireYesNo(Messenger var1) {
    ...
    this.d = CommandSession.addSession(
        this, Duration.ofSeconds(30L), (var1x) -> {
  ...
  public void sendMessage(@NotNull CommandSender messageReceiver, @NotNull Messenger messenger, Object... edits) {
    if (this.loggingLevel.atLeast(
        super.sender == messageReceiver ? CommandLoggingLevel.ALL : CommandLoggingLevel.INFO)) {
  ...
  public Boolean parseBool(@IntRange(from = 0L, to = 5L) int var1) {
    String var2;
    if (!(var2 = this.arg(var1).toLowerCase(Locale.ENGLISH)).equals("true") && !AuspiceLang.TRUE.parse(
        super.sender).toLowerCase().equals(var2)) {
  ...
  public CommandContext debug(@NotNull String message) {
  ...
  private boolean a(String var1, AuspiceLang var2) {
    String[] var6 = Strings.splitArray(var2.parse(super.messageSettings), ' ');
  ...
  public boolean assertEnabledWorld() {
    if (!this.isPlayer()) {
      ...
      String var1 = this.senderAsPlayer().getWorld().getName();
  ...
  public @NotNull CommandContext var(@NotNull String name, @Nullable Object value) {
  ...
  public AnyNumber getNumber(int var1, boolean var2, boolean var3, Messenger var4) {
    Pair var5;
    if ((var5 = this.getNumber(
        var1, var4, FunctionalList.create().addIf(var2, NumberConstraint.INTEGER_ONLY).addIf(
            var3,
            NumberConstraint.POSITIVE
        ).toArray(new NumberConstraint[0])
    )).getValue() != null) {
  ...
  public Pair<AnyNumber, Messenger> getNumber(int var1, Messenger var2, NumberConstraint... var3) {
    if (var1 >= 0 && var1 < this.args.length) {
      ...
      return ProcessToMessage.getNumber(
          (new NumberProcessor(this.arg(var1))).withAllDecorators().withConstraints(var3));
    } else {
      throw new IllegalArgumentException(
          "Cannot get parameter at index " + var1 + " with parameters: (" + this.args.length + ')' + Arrays.toString(
              this.args));
    }
  }

  public AuspicePlayer getAuspicePlayer() {
  ...
  @Override
  @NotNull
  public CommandSender messageReceiver() {
    return super.sender;
  }

  public boolean hasPermission(DefaultAuspicePluginPermissions var1) {
    return this.hasPermission(var1, true);
  }

  public boolean hasPermission(DefaultAuspicePluginPermissions var1, boolean var2) {
    return var1.hasPermission(super.sender, var2);
  }

  public boolean isAdmin() {
    return !this.isPlayer() || this.getAuspicePlayer().isAdmin();
  }

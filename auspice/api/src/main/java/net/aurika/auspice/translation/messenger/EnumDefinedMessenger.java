...
import net.aurika.configuration.annotations.AdvancedMessage;
import net.aurika.configuration.annotations.Comment;
import net.aurika.configuration.annotations.Path;
import net.aurika.configuration.annotations.RawPath;
...
public interface EnumDefinedMessenger extends DefinedMessenger {
  ...
  default void sendMessage(@NotNull CommandSender messageReceiver, @NotNull MessageContextImpl textPlaceholderProvider) {
    ...
    Object var4 = messageReceiver instanceof Player ? LanguageManager.localeOf(
        messageReceiver) : textPlaceholderProvider.diversity();
    Objects.requireNonNull(var4);
    Object var3 = var4;
    textPlaceholderProvider.lang((Locale) var4);
    if (CommandAdminTrack.isTracking(
        messageReceiver) && this != AuspiceLang.COMMAND_ADMIN_TRACK_TRACKED && this != AuspiceLang.COMMAND_ADMIN_TRACK_GUI) {
      String var5;
      Object var8;
      label23:
      {
        SupportedLanguage var6 = (SupportedLanguage) var4;
        var5 = StringsKt.replace$default(
            Auspice.get().getDataFolder().toPath().relativize(var6.getMainLanguageFile()).toString(), ' ', '*', false,
            4, (Object) null
        );
      ...
      var4 = var8;
      AuspiceLang.COMMAND_ADMIN_TRACK_TRACKED.getProvider((Locale) var3).send(
          messageReceiver, (new MessageContextImpl()).parse(
              "path",
              Strings.join(this.translationEntry().getPath(), " {$sep}-> {$s}")
          ).raw("file", var5).raw(
              "raw", LanguageManager.getRawMessage(this, (SupportedLanguage) var3)).parse("line", var4)
      );
    }
  ...
  static @NotNull ConfigEntry getEntry(@Nullable String var1, @NotNull EnumDefinedMessenger messenger, int[] group) {
    ...
    if (pathAnn != null) {
      ...
      try {
        List<String> var11 = Strings.split(
            Strings.getGroupedOption(messenger.name(), Arrays.copyOf(var7, var7.length)), '.', false);
        ...
        throw new RuntimeException(
            var10002.append(var10003).append(' ').append(messenger).append(" -> ").append(
                ArraysKt.joinToString(group, ", ", "", "", -1, "", null)).toString(), var6
        );
      }
    }
  }

  ...
  public static void register(@NotNull AuspiceUser user) {
    ...
    if (!NSKey.NAMESPACE_PATTERN.matcher(user.namespace()).matches()) {
      throw new IllegalArgumentException(
          "namespace of auspice uer dosen't matches pattern: " + NSKey.ALLOWED_NAMESPACE);
    }

    AuspiceUser other_name = BY_NAME.put(user.auspiceUserName(), user);
    if (other_name != null) {
      (new IllegalStateException(
          "Two Auspice user registered a name: '" + user.auspiceUserName() + "', " + other_name + '|' + user)).printStackTrace();
    }

    AuspiceUser other_ns = BY_NAME.put(user.auspiceUserName(), user);
    if (other_ns != null) {
      (new IllegalStateException(
          "Two Auspice user registered a namespace: '" + user.auspiceUserName() + "', " + other_ns + '|' + user)).printStackTrace();
    }

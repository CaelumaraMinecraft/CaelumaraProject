...
public class Vector3Location implements WorldContainer, Point3D {
    ...
    public boolean equals(@Nullable Object other) {
        if (!(other instanceof Vector3Location)) {
            ...
            return Intrinsics.areEqual(this.getWorld(), ((Vector3Location) other).getWorld()) && this.getX() == ((Vector3Location) other).getX() && this.getY() == ((Vector3Location) other).getY() && this.getZ() == ((Vector3Location) other).getZ();
        }
    }
    ...
    private Vector3Location simpleAdd(Number x, Number y, Number z) {
        return of(this.getWorld(), this.getX() + x.doubleValue(), this.getY() + y.doubleValue(), this.getZ() + z.doubleValue());
    }

...
import net.aurika.snakeyaml.extension.validation.ValidationFailure.Severity;
...
public class StandardValidator implements NodeValidator {
    ...
    public ValidationFailure validate(ValidationContext context) {
        if (!(context.getNode() instanceof ScalarNode)) {
            return context.fail(new ValidationFailure(Severity.ERROR, context.getNode(), context.getRelatedKey().getStartMark().get(), "Wrong type, expected '" + this.type.name + "' but got '" + context.getNode().getTag().getValue() + '\''));
            ...
            if (type == null) {
                ...
                if (this.minLen != 0 || this.maxLen != 0) {
                    int length;
                    if (this.type == StandardValidator.Type.STR) {
                        length = NodesKt.getParsed(scalarNode).toString().length();

                    } else {
                        length = ((Number) NodesKt.getParsed(scalarNode)).intValue();
                    }

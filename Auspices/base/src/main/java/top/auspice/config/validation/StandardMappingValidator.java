...
public class StandardMappingValidator implements ConfigValidator {
    ...
    public ValidationFailure validate(ValidationContext context) {
        ...
        if (!(parsed instanceof Map<?, ?>)) {
            return this.optional && section.getLabel() == Label.NULL ? null : context.err("Expected a mapping section, instead got " + section.getBranchState().name().toLowerCase(Locale.ENGLISH));
        } else {
                        ...
                        do {
                            if (!subSecEntryIt.hasNext()) {
                                if (requiredKeys != null && !requiredKeys.isEmpty()) {
                                    return context.err("Missing required entries " + Arrays.toString(requiredKeys.toArray()) + " ");  //todo inPresent()
                                }

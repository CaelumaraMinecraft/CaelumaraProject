...
public class FixedValuedValidator implements ConfigValidator {
    ...
    public ValidationFailure validate(ValidationContext context) {
        ...
        if (scaVal == null) {
            return context.err("Expected a simple scalar value here, but got a " + section.getLabel().getValue() + " instead");
        } else {
            String val = scaVal.toLowerCase(Locale.ENGLISH);
            return this.acceptedValues.contains(val) ? null : context.err("Unexpected value '" + scaVal + "' expected one of " + Arrays.toString(this.acceptedValues.toArray()));
        }

...
import top.auspice.config.yaml.snakeyaml.common.NodeReference.Type;
    ...
    private void replaceAll0() {
        ...
        switch (this.node.getNodeType()) {
                        ...
                        if (!this.isIgnored(new ReplacementDetails(this.path, element, false, seqNode))) {
                            if (this.variables != null && !this.variables.isEmpty() && element instanceof ScalarNode) {
                                ...
                                if (value instanceof SequenceNode mergingValues) {
                                    newElements.addAll(mergingValues.getValue().stream().map(NodeUtils::cloneNode).toList());
                                    this.changed = true;
                                    continue;
                                }
                            }
            ...
            case MAPPING:
                ...
                Node competeValue;
                for(Iterator<NodeTuple> var7 = map.getValue().iterator(); var7.hasNext(); newPairs.put(completeKey.getValue(), new NodeTuple(completeKey, competeValue))) {
                    ...
                    if (!this.isIgnored(new ReplacementDetails(newPath, pair.getKey(), true, (SequenceNode)null))) {
                        ...
                        if (!(competeValue instanceof ScalarNode)) {
                            throw new RuntimeException("Mapping key must be a scalar node at path '" + newPath + "' for " + pair.getKey() + " -> " + competeValue);
                        }

...
import top.auspice.config.sections.ConditionalConfigSection;
import top.auspice.config.sections.ConfigSection;
import top.auspice.config.sections.interpreter.SectionInterpretContext.Result;
...
public interface SectionInterpreter<T> {
    ...
    SectionInterpreter<java.awt.Color> COLOR = new SectionInterpreter<Color>() {
        ...
        private static Color a(ConfigSection colorSection) {
            Integer R = colorSection.getInteger(new String[]{"r"}) == null ? colorSection.getInteger(new String[]{"R"}) : null;
            Integer G = colorSection.getInteger(new String[]{"g"}) == null ? colorSection.getInteger(new String[]{"G"}) : null;
            Integer B = colorSection.getInteger(new String[]{"b"}) == null ? colorSection.getInteger(new String[]{"B"}) : null;
            Integer A = colorSection.getInteger(new String[]{"a"}) == null ? colorSection.getInteger(new String[]{"A"}) : null;
    ...
    static Duration getTime(@Nullable ConfigSection section, @Nullable PlaceholderProvider placeholderProvider) {
        if (section == null) {
            ...
            if (constructed instanceof Number) {
                ...
                return constructed instanceof Duration ? (Duration) constructed : Duration.ofMillis((long) MathUtils.eval((MathCompiler.Expression) constructed, placeholderProvider == null ? PlaceholderProvider.EMPTY : placeholderProvider));
            }
        }
    }

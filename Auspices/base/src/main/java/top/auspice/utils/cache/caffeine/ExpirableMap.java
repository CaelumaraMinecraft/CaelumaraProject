...
public final class ExpirableMap<K, V> implements Cache<K, V> {
    ...
    @Nullable
    public V getIfPresent(@CompatibleWith("K") @NotNull K key) {
        Objects.requireNonNull(key);
        ReferencedExpirableObject<V> var10000 = this.cache.getIfPresent(key);
    ...
    @NotNull
    public V get(@NotNull final K key, @NotNull final Function<? super K, ? extends V> mappingFunction) {
        Objects.requireNonNull(key);
        Objects.requireNonNull(mappingFunction);
        ReferencedExpirableObject<V> var10000 = this.cache.get(key, (dwa) -> {
    ...
    public Map<K, V> getAllPresent(@NotNull Iterable<? extends K> keys) {
        ...
        Map<K, ReferencedExpirableObject<V>> var10000 = this.cache.getAllPresent(keys);
        ...
        while (var5.hasNext()) {
            ...
            Pair<K, V> var11 = TuplesKt.to(it.getKey(), it.getValue().getReference());

...
import top.auspice.utils.internal.functional.SecondarySupplier;
import top.auspice.utils.internal.string.QuantumString;
...
public class CommandTabContext extends CommandContext {
    ...
    public @NonNull List<String> getPlayers(int var1, Predicate<OfflinePlayer> var2) {
        return this.getPlayers(Server.get().getPlayerManager().getOnlinePlayers(), this.arg(var1), false, var2);
    }
    @NonNull
    ...
    private String a(Messenger var1) {
        TextObject var4 = var1.getMessageObject(super.messageSettings.getLanguage());
        ...
        try {
            ...
            super.messageSettings.dontIgnoreColors();
        }
    ...
    public Messenger tabParameter(String var1, boolean var2) {
        ...
        return DefaultedMessenger.oneOf(this.getCommand().lang("tab-completion-" + (var2 ? "required" : "optional"), var1), () -> this.getCommand().lang("tab-completion", var1), () -> new StaticMessenger("{$sep}" + var3 + var5 + var1 + "{$sep}" + var4));
    }
    ...
    public Completer tabCompleteNumberModifier(String var1, NumberConstraint... var2) {
        ...
        if (this.isAtCurrentParameter()) {
            ...
            if (this.getNumberModifierType(var9) == null) {
                ...
                List var8;
                if ((var8 = this.suggest(var3, Stream.of(AuspiceLang.SETTERS_TYPE_ADD, AuspiceLang.SETTERS_TYPE_SUBTRACT, AuspiceLang.SETTERS_TYPE_SET).flatMap((var1x) -> Arrays.stream(Strings.splitArray(var1x.parse(super.messageSettings), ' '))).collect(Collectors.toList()))).isEmpty() && !var9.isEmpty()) {

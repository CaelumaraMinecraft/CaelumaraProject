...
import top.auspice.configs.texts.compiler.builders.context.TextBuilderContextProvider;
...
import static top.auspice.configs.texts.compiler.builders.context.ComplexTextBuilderContextProvider.*;
...
public abstract class TextPiece implements AbstractTextPiece {
    ...
    public void build(TextBuilderContextProvider builderProvider) {
        ...
        throw new UnsupportedOperationException("Unsupported " + TextBuilderContextProvider.class.getSimpleName() + ": " + builderProvider);
    }
    ...
    public static final class Hover extends TextPiece {
        ...
        public void build(ComplexTextBuilderContextProvider builderProvider) {
            PlainTextBuilderContextProvider clickActionBuilder = new PlainTextBuilderContextProvider(builderProvider.getSettings());

            for (TextPiece textPiece : this.clickAction) {
                textPiece.build(clickActionBuilder);
            }

            ClickEvent clickEvent = this.action == null ? null : ClickEvent.clickEvent(this.action, clickActionBuilder.merge());

            // 复用文本为空的 TextComponent.Builder
            TextComponent.Builder plainTextBuilder = builderProvider.getCurrentComponentBuilder().content().isEmpty() ? builderProvider.getCurrentComponentBuilder() : builderProvider.newComponentBuilder(FORMATTING_MERGES);

            ComplexTextBuilderContextProvider normalMessageBuilder = new ComplexTextBuilderContextProvider(builderProvider, plainTextBuilder, builderProvider.getSettings());
            ...
            ComplexTextBuilderContextProvider hoverMessageBuilder = new ComplexTextBuilderContextProvider(builderProvider, builderProvider.getSettings());
            ...
            HoverEvent<Component> hoverEvent = HoverEvent.hoverEvent(HoverEvent.Action.SHOW_TEXT, hoverMessageBuilder.getCurrentComponentBuilder().build());  // TODO validate
        ...
        public void build(HTMLTextBuilderContextProvider builderProvider) {
            HTMLTextBuilderContextProvider newBuilder = new HTMLTextBuilderContextProvider(builderProvider.getSettings());
        ...
        public int jsonLength() {
            return json_template + Arrays.stream(this.normalMessage).mapToInt(TextPiece::jsonLength).sum() + Arrays.stream(this.hoverMessage).mapToInt(TextPiece::jsonLength).sum() + Arrays.stream(this.clickAction).mapToInt(TextPiece::jsonLength).sum();
        }
        ...
        public String toString() {
            ...
            return "Hover{ length=" + this.length() + " | " + Arrays.toString(this.normalMessage) + ';' + Arrays.toString(this.hoverMessage) + ';' + var1 + " }";
        }
    }
    ...
    public static final class ColorAccessor extends Color {
        ...
        public List<TextPiece> getLastColors(TextPlaceholderProvider var1) {
            ArrayList<TextPiece> var2 = new ArrayList<>(3);
            Object placeholder = this.variable.getPlaceholder(var1);
            if (placeholder == null) {
                return Collections.singletonList(new Plain("{" + this.variable.value.asString(true) + " & " + this.range + '}'));
            } else {
                TextObject var4;
                if (placeholder instanceof TextObject) {
                    ...
                    if (!(placeholder instanceof PlaceholderTranslationContext)) {
                        var2.add(new Plain("{" + this.variable.value.asString(true) + " & " + this.range + " (this special placeholder which is of type" + placeholder.getClass() + " -> " + placeholder + " is not supported for chat accessors}"));
                        return var2;
                    }
        ...
        public void build(ComplexTextBuilderContextProvider builderProvider) {
            ...
            for (TextPiece vapiece3 : this.getLastColors(builderProvider.getSettings())) {
        ...
        public void build(PlainTextBuilderContextProvider builderProvider) {
            ...
            for (TextPiece var3 : this.getLastColors(builderProvider.getSettings())) {
    ...
    public static final class Variable extends TextPiece {
        ...
        public void build(ComplexTextBuilderContextProvider builderProvider) {
            Object placeholder = this.getPlaceholder(builderProvider.getSettings());
            if (placeholder == null) {  // 构建占位符失败
                builderProvider.newComponentBuilder(ALL_MERGES).content(this.value.asString(true));
            } else {
                builderProvider.getSettings().usePrefix(false);
                if (placeholder instanceof MessageObjectBuilder) {
                    ((MessageObjectBuilder) placeholder).build(builderProvider);
        ...
        public TextObject getCompiled(PlaceholderTranslationContext ptc) {
            ...
            TextCompilerSettings var3 = ptc.getSettings();
            return TextCompiler.compile(var1.toString(), var3);
        }

        public void build(PlainTextBuilderContextProvider builderProvider) {
            Object placehholder = this.getPlaceholder(builderProvider.getSettings());
            if (placehholder == null) {
                builderProvider.getCurrentLine().append(this.value.asString(true));
            } else {
                builderProvider.getSettings().usePrefix(false);
                if (placehholder instanceof MessageObjectBuilder) {
                    ((MessageObjectBuilder) placehholder).build(builderProvider);
        ...
        public Object getPlaceholder(TextPlaceholderProvider var1) {
            Object var2 = this.value.request(var1);
            return PlaceholderTranslationContext.unwrapContextualPlaceholder(this.value.applyModifiers(var2), var1);
        }
    ...
    public static final class HexTextColor extends Color {
        ...
        private CodedChatFormats approximateCode;
        ...
        public void build(PlainTextBuilderContextProvider builderProvider) {
            if (this.approximateCode == null) {
                CodedChatFormats namedFormat = BaseColorUtils.hexColorToStd(this.color);
        ...
        public CodedChatFormats getApproximateCode() {
            if (this.approximateCode == null) {
    ...
    public static final class SimpleFormat extends Color {
        private final CodedChatFormats code;
        private static final int b = 11;

        public SimpleFormat(CodedChatFormats code) {
            this.code = code;
        }
        ...
        public void build(ComplexTextBuilderContextProvider builderProvider) {
            if (this.code.isColor()) {
                ...
                newBuilder.color((TextColor) this.code.getValue());
            }
            ...
            if (this.code.isDecoration()) {
                ...
                newBuilder.decoration((TextDecoration) this.code.getValue(), TextDecoration.State.TRUE);
            }
        }

        public void build(PlainTextBuilderContextProvider builderProvider) {
            builderProvider.getCurrentLine().append('§').append(this.code.getCode());
        }
        ...
        public void build(HTMLTextBuilderContextProvider builderProvider) {
            ...
            } else if (this.code == CodedChatFormats.RESET) {
                builderProvider.newElement(false);
            } else {
                if (!builderProvider.getCurrentElement().isEmpty()) {
        ...
        public CodedChatFormats getColor() {
            return this.code;
        }
    ...
    public static final class Null extends TextPiece {
        ...
        public static TextPiece[] getArrayInstance() {
    ...
    public static final class Conditional extends TextPiece {
        ...
        public TextPiece[] getPiece(PlaceholderContextBuilder var1) {
            ...
            do {
                if (!var2.hasNext()) {
                    return TextPiece.Null.getArrayInstance();
                }
        ...
        public void addFor(TextBuilderContextProvider builderProvider) {
            TextPiece[] pieces = this.getPiece(builderProvider.getSettings());
            if (pieces != null) {
        ...
        private int a(Function<TextPiece, Integer> var1) {
            return this.branches.stream().map(Pair::getValue).map((var1x) -> Arrays.stream(var1x).map(var1).reduce(Integer::sum).orElse(0)).max(Integer::compareTo).orElse(0);
        }
    ...
    public static class Font extends TextPiece {
        private final @NotNull @KeyPattern String

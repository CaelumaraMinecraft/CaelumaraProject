...
final class JarRelocatorTask {
  ...
  void processEntries() throws IOException {
    ...
    while (entries.hasMoreElements()) {
      ...
      if (!name.equals("META-INF/INDEX.LIST") && !entry.isDirectory() && !SIGNATURE_FILE_PATTERN.matcher(
          name).matches()) {
  ...
  private void processEntry(@NotNull JarEntry entry, InputStream entryIn) throws IOException {
    String name = entry.getName();
  ...
  private void processDirectory(@NotNull String name, boolean parentsOnly) throws IOException {
  ...
  private static void copy(@NotNull InputStream from, OutputStream to) throws IOException {

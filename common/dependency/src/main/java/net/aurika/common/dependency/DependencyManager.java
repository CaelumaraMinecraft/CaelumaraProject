...
import net.aurika.common.dependency.relocation.RelocationHandler;
...
public final class DependencyManager {
  ...
  public DependencyManager(@NotNull BootstrapProvider plugin) {
    this.plugin = plugin;
  ...
  public void tryLoadDependency(CountDownLatch latch, Dependency dependency) {
    ...
    do {
      ...
      try {
        ...
        if (!(ex instanceof DependencyDownloadException) || !(ex.getCause() instanceof SocketException) || !ex.getCause().getMessage().equalsIgnoreCase(
            "Connection reset")) {
          throw new IllegalStateException("Unable to load dependency: " + dependency.name(), ex);
        }

        this.plugin.getLogger().warning(
            "[" + retries + "] Failed to download dependency '" + dependency + "': " + ex.getMessage() + ". Retrying...");
        count = false;
      } finally {
        if (count) {
          latch.countDown();
    ...
    if (Files.exists(file)) {
      ...
      if (notifyDownload) {
        this.plugin.getLogger().info(
            "Downloading " + dependency + " v" + dependency.getDefaultVersion().getVersion() + "...");
        this.plugin.runAsyncLater(
            () -> {
              if (!done.get()) {
//                        DependencyRepository.MAVEN_CENTRAL.getUrl() + dependency.getMavenRepoPath();
                this.plugin.getLogger().warning(
                    "It looks like downloading " + dependency.name() + " is taking longer than expected. In case you have have any connection issues on your side, here's the direct download link: " + DependencyRepository.MAVEN_CENTRAL.getURLFor(
                        dependency) + "\nAfter downloading it, put it inside '" + this.plugin.getLibsFolder().toAbsolutePath() + "' folder and make sure the file name is '" + dependency.getFileName(
                        null) + "' (if not rename it) then restart the server.");
              }
      ...
      this.plugin.runAsyncLater(
          () -> {
            if (!done.get()) {
              this.plugin.getLogger().warning(
                  "It appears that the server is stuck trying to download " + dependency + ". Please try restarting the server.");
            }
          }, 5L, TimeUnit.MINUTES
      );
  ...
  static {
    REQUIRED_DEPENDENCIES = Arrays.asList(
        Dependency.KOTLIN_STDLIB, Dependency.CAFFEINE, Dependency.XSERIES, Dependency.GSON, Dependency.GUAVA);
  }
}

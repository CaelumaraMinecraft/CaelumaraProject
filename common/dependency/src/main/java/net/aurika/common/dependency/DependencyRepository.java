  ...
  public byte[] download(Dependency dependency) throws DependencyDownloadException {
    ...
    if (!dependency.getDefaultVersion().checksumMatches(hash)) {
      throw new DependencyDownloadException(
          "Downloaded file had an invalid hash. Expected: " + Base64.getEncoder().encodeToString(
              dependency.getDefaultVersion().getChecksum()) + ' ' + "Actual: " + Base64.getEncoder().encodeToString(
              hash));
    } else {
      return bytes;
    }

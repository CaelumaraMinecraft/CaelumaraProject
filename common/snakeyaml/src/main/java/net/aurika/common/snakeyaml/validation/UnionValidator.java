...
public class UnionValidator implements NodeValidator {
  ...
  public ValidationFailure validate(ValidationContext context) {
    ...
    for (NodeValidator validator : this.union) {
      ...
      ValidationContext innerContext = new ValidationContext(
          context.getRelatedKey(), context.getNode(), context.getValidatorMap(), capturedFails);
    ...
    return context.fail(new ValidationFailure(
        Severity.ERROR, context.getNode(), null,
        "None of the types matched: " + (String) allFails.stream().map(ValidationFailure::getMessage).collect(
            Collectors.joining(", "))
    ));
  }

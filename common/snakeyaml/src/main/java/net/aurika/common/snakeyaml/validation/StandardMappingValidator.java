...
public class StandardMappingValidator implements NodeValidator {
  ...
  public ValidationFailure validate(ValidationContext context) {
    ...
    if (!(context.getNode() instanceof MappingNode mapping)) {
      return this.optional && context.getNode().getTag() == Tag.NULL ? null : context.err(
          "Expected a mapping section, instead got " + context.getNode().getClass().getSimpleName().toLowerCase(
              Locale.ENGLISH));
    } else {  //若为MappingNode
      ...
      while (true) {
            ...
            do {
              if (!iterator.hasNext()) {
                if (requiredKeys != null && !requiredKeys.isEmpty()) {
                  return context.err("Missing required entries " + Arrays.toString(requiredKeys.toArray())).withMarker(
                      context.getRelatedKey() == null ? new Mark(
                          "[ROOT]", 0, 0, 0, new char[0],
                          0
                      ) : context.getRelatedKey().getStartMark().get());  //todo inPresent()
                }
              ...
              if (this.keyValidator != null) {
                ValidationFailure failure = this.keyValidator.validate(
                    context.delegate(pair.getKeyNode(), pair.getKeyNode()));
                if (failure != null) {
        ...
        } while (this.valueValidatorKeys != null && !this.valueValidatorKeys.contains(

package net.aurika.common.snakeyaml.node.interpret;
...
import static net.aurika.common.snakeyaml.node.interpret.NodeInterpretContext.Result.*;
...
public interface NodeInterpreter<T> {
  ...
  @NotNull NodeInterpreter<Boolean> BOOLEAN = NodeInterpreter::parseBoolean;
  ...
  static Boolean parseBoolean(@NotNull NodeInterpretContext<Boolean> context) {
  ...
  private static String string(NodeInterpretContext<String> context) {
    ...
    if (node == null) {
      ...
      if (node instanceof ScalarNode) {
        ScalarNode scalarNode = (ScalarNode) node;
        if (NodesKt.parsed(node) instanceof CharSequence) {
          CharSequence chSeqParsed = (CharSequence) NodesKt.parsed(node);
  ...
  private static List<String> string_list(NodeInterpretContext<List<String>> context) {
    ...
    if (node == null) {
      ...
      if (!(node instanceof SequenceNode seqNode)) {
        return node instanceof ScalarNode ? Strings.split(

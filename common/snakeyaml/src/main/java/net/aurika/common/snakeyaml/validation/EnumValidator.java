...
public class EnumValidator implements NodeValidator {
  ...
  public ValidationFailure validate(ValidationContext context) {
    if (!(context.getNode() instanceof ScalarNode scalarNode)) {
      return context.err(
          "Expected a " + this.enumerator.getSimpleName() + " type, but got an option of type '" + context.getNode().getTag().getValue());
    } else {
      ...
      try {
        ...
        return context.err(
            "Expected a " + this.enumerator.getSimpleName() + " type, but got '" + scalarNode.getValue() + '\'');
      }
    }
  }

...
public class FixedValuedValidator implements NodeValidator {
  ...
  public ValidationFailure validate(ValidationContext context) {
    if (!(context.getNode() instanceof ScalarNode scalarNode)) {
      return context.err(
          "Expected a simple scalar value here, but got a " + context.getNode().getTag().getValue() + " instead");
    } else {
      String val = scalarNode.getValue().toLowerCase(Locale.ENGLISH);
      return this.acceptedValues.contains(val) ? null : context.err(
          "Unexpected value '" + scalarNode.getValue() + "' expected one of " + Arrays.toString(
              this.acceptedValues.toArray()));
    }
  }

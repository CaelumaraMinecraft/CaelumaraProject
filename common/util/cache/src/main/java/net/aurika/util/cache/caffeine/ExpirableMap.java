...
public final class ExpirableMap<K, V> implements Cache<K, V> {
  ...
  public ExpirableMap(@Nullable ExpirationStrategy defaultExpirationStrategy) {
    ...
    Cache<K, ReferencedExpirableObject<V>> var10001 = CacheHandler.typedBuilder().expireAfter(
        new ExpirableObjectExpiry<>()).build();
  ...
  public V get(@NotNull K key, @NotNull Function<? super K, ? extends V> mappingFunction) {
    ...
    ReferencedExpirableObject<V> var10000 = cache.get(
        key, (dwa) -> {
          return new ReferencedExpirableObject<>(
              mappingFunction.apply(key), ExpirableMap.this.getAssertDefaultStrategy());
        }
    );
    Objects.requireNonNull(var10000);

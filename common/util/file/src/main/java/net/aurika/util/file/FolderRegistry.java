      ...
      try {
        ...
        try {
          var1.filter(Files::isRegularFile).filter(this::filter).forEach((var1x) -> {
            ...
            this.a.add(var3);
            AuspiceLogger.debug(
                AuspiceDebug.FOLDER_REGISTRY, "Visting present of " + this + " -> " + var3 + " | " + var2);

            try {
              this.handle(new Entry(var3, var2, var1x, null));
            } catch (Throwable var4) {
              throw new IllegalStateException(
                  "Error while registering " + this.displayName + " '" + var3 + "' " + var1x.toFile(), var4);
            }
          });
        } catch (Throwable var4) {
          if (var1 != null) {
          ...
          try {
            pathStream.filter(Files::isRegularFile).filter(this::filter).forEach((var1x) -> {
              ...
              if (!this.a.contains(var3)) {
                AuspiceLogger.debug(
                    AuspiceDebug.FOLDER_REGISTRY, "Visting default of " + this + " -> " + var3 + " | " + var2);
                ...
                try {
                  ...
                  throw new IllegalStateException(
                      "Error while registering default " + var3 + " (" + var2 + ") for " + this, var4);
                }
              }
            });

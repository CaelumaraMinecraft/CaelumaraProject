      ...
      try {
        if (var1 != null) {
          try {
            ...
            this.defaults = YamlNodeSection.root(YamlContainer.parse(
                (new YamlParseContext()).named(var2).shouldLoadAliases(this.loadAnchors).stream(var1)));
          } catch (ParserException | ComposerException | UnsupportedOperationException |
                   ScannerException var4) {
            AuspiceLogger.error(
                "Failed to load defaults for config '" + this.file.getAbsolutePath() + "' from '" + this.getDefaultsPath() + "':");
    ...
    if (this.file == null) {
      ...
      if (this.file.exists()) {
        try {
          ...
          this.config = YamlNodeSection.root(YamlContainer.parse(
              (new YamlParseContext()).named(this.file.getName()).shouldLoadAliases(this.loadAnchors).stream(var1)));
          var1.close();
        } catch (ParserException | ComposerException | UnsupportedOperationException |
    ...
    if (!this.isDefault()) {
      if (this.defaults == null) {
        throw new IllegalStateException(
            "The config " + this.file.getName() + " cannot be updated because there's no default config for it");
      } else {
        ConfigManager.beforeWrite(this);
        ...
        try {
          var1 = Updater.updateConfig(
              this.config.getRootNode(), this.defaults.getRootNode().clone(), this.schema, this.file.toPath(),
              new Dump(new DumpSettings())
          );
        } catch (Throwable var7) {
          AuspiceLogger.error(
              "An error occurred while attempting to update " + this.file.getAbsolutePath() + ": " + var7.getMessage() + (var7.getMessage().contains(
                  "another process") ? ". This is probably caused by your text editor" : ""));
          return;
        }

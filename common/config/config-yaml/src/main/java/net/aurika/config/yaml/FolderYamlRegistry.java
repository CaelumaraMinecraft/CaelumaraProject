...
import net.aurika.config.yaml.adapter.YamlFile;
import net.aurika.config.yaml.adapter.YamlResource;
    ...
    for (Map.Entry<String, YamlContainer> stringYamlContainerEntry : this.c.entrySet()) {
      ...
      try {
        ...
        throw new IllegalStateException(
            "Error while registering default " + super.displayName + " '" + stringYamlContainerEntry.getKey() + "' " + stringYamlContainerEntry.getValue().getFile(),
            var5
        );
      }
    }

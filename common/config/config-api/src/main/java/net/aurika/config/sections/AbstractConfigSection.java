...
public abstract class AbstractConfigSection implements ConfigSection {
  ...
  public AbstractConfigSection(@NotNull ConfigSection parent, @NotNull String key, @NotNull Label label) {
    this(
        Validate.Arg.notNull(
            parent, "parent",
            "Parent section must be provided when auto generate path."
        ).getPath().append(Validate.Arg.notNull(key, "key", "Section name must be provided.")), key, parent, label
    );
  }

package net.aurika.utils.snakeyaml.nodes.interpret;
...
import static net.aurika.utils.snakeyaml.nodes.interpret.NodeInterpretContext.Result.*;
...
public interface NodeInterpreter<T> {
    ...
    /**
     * 标点严格, 且不使用 {@link NodesKt#getParsed(Node)}
     */
    ...
    /**
     * 标点不严格, 且当 {@link NodesKt#getParsed(Node)} 返回 {@link CharSequence} 时优先使用
     */
    private static String string(NodeInterpretContext<String> context) {
        ...
        if (node == null) {
            ...
            if (node instanceof ScalarNode scalarNode) {
                if (NodesKt.getParsed(node) instanceof CharSequence chSeqParsed) {

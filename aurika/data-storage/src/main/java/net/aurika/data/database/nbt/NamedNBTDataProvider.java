    ...
    public <E, C extends Collection<E>> @NotNull C asCollection(@NotNull C c, @NotNull BiConsumer<C, SectionableDataGetter> dataProcessor) {
        ...
        if (tag != null) {
            for (BinaryTag eTag : ((ListBinaryTag) tag)) {
                dataProcessor.accept(c, createProvider$core(eTag));
            }
        }
        return c;
    }
    ...
    public <K, V, M extends Map<K, V>> @NotNull M asMap(@NotNull M m, @NotNull TriConsumer<M, DataGetter, SectionableDataGetter> dataProcessor) {
        ...
        if (compound != null) {
            for (Map.Entry<String, ? extends BinaryTag> entry : compound) {
                dataProcessor.accept(m, new NBTDataProvider(StringBinaryTag.stringBinaryTag(entry.getKey())), createProvider$core(entry.getValue()));
            }
        }
        return m;
    }

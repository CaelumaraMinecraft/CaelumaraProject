package net.aurika.kingdoms.powerfulterritory.constant.invade_protection;
...
public class StandardInvadeProtection implements InvadeProtection {
  ...
  private static StandardInvadeProtection a(boolean b, String nsKey, String defaultName) {
    StandardInvadeProtection a = new StandardInvadeProtection(
        b,
        MessengerUtil.createMessenger(
            new String[]{PowerfulTerritoryAddon.KEY, "invade-protection-name", nsKey.toLowerCase(
                Locale.ENGLISH).replace('_', '-')}, defaultName
        ),

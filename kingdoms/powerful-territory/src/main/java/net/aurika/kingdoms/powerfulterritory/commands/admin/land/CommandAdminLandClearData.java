...
import net.aurika.kingdoms.powerfulterritory.data.InvadeProtectionMetaHandler;
import net.aurika.kingdoms.powerfulterritory.data.LandCategoryMetaHandler;
import net.aurika.kingdoms.powerfulterritory.data.LandContractionsMetaHandler;
import net.aurika.kingdoms.powerfulterritory.util.PowerfulTerritoryLogger;
...
public class CommandAdminLandClearData extends KingdomsCommand {
  ...
  static {
    register(
        "PowerfulTerritory:land-contractions",
        (land) -> land.getMetadata().remove(LandContractionsMetaHandler.INSTANCE)
    );
    register("PowerfulTerritory:land-category", (land -> land.getMetadata().remove(LandCategoryMetaHandler.INSTANCE)));
    register(
        "PowerfulTerritory:invade-protection",
        (land -> land.getMetadata().remove(InvadeProtectionMetaHandler.INSTANCE))
    );
  }

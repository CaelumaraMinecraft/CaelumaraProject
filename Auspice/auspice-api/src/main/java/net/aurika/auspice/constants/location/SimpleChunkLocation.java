...
import net.aurika.auspice.server.location.BlockPoint2D;
import net.aurika.auspice.server.location.BlockVector2;
import net.aurika.auspice.server.location.Direction;
...
public class SimpleChunkLocation implements Cloneable, DataStringRepresentation, BlockPoint2D, StructuredDataObject {

  private final @NonNull String world;
  private final int x;
  private final int z;

  public static final FunctionsDataStructSchema<SimpleChunkLocation> DATA_SCHEMA = FunctionsDataStructSchema.of(
      ...
      SimpleChunkLocation::fromDataString,
      SimpleChunkLocation::asDataString,
  ...
  public static SimpleChunkLocation of(Location var0) {
    ...
    return new SimpleChunkLocation(
        (Objects.requireNonNull(var0.getWorld(), "Simple chunk location cannot have a null world")).getName(),
  ...
  public boolean isInChunk(@NonNull SimpleBlockLocation var1) {
    ...
    return (var2 = var1.toSimpleChunkLocation()).x == this.x && var2.z == this.z && Objects.equals(
        this.world, var2.world);
  }
  ...
  public double distanceIgnoreWorld(@NonNull SimpleChunkLocation var1) {
    return Math.sqrt(NumberConversions.square((double) this.x - (double) var1.x) + NumberConversions.square(
        (double) this.z - (double) var1.z));
  }

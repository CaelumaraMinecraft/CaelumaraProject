...
public final class PlaceholderLexer {
      ...
      if (((CharToken) var1).getType() == PlaceholderLexer.CharTokenType.CLOSURE) {
        if (!this.c) {
          throw new IllegalStateException(
              "Not expecting to look for placeholder closure, but found one at " + this.index + " -> " + new String(
                  this.chars));
        }
  ...
  public static CharToken access$getCharToken(PlaceholderLexer placeholderLexer, char c) {
    CharTokenType charTokenType = c == '*' ? CharTokenType.POINTER : (c == '_' ? CharTokenType.SEPARATOR : (c == ':' ? CharTokenType.FUNCTION_START : (c == '=' ? CharTokenType.FUNCTION_ARGUMENT_EQUAL : (c == ',' ? CharTokenType.FUNCTION_ARGUMENT_SEPARATOR : (c == '@' ? CharTokenType.MODIFIER : (c == '%' ? CharTokenType.CLOSURE : (CharsKt.isWhitespace(
        c) ? CharTokenType.WHITESPACE : null)))))));

...
public abstract class ExpressionBasedTopData<K, V> extends IndexedTopData<K, V> {
  ...
  public ExpressionBasedTopData(MathCompiler.Expression var1, ConditionCompiler.LogicalOperand var2, String dataName, Messenger var4, Messenger var5) {
    ...
    this.filter = var2;
    this.predicate = var2 == null ? null : (var2x) -> ConditionProcessor.process(
        var2, this.getPlaceholderProvider(var2x));
  ...
  protected abstract VariableProvider getPlaceholderProvider(V var1);
  ...
  public String toString() {
    return this.getClass().getSimpleName() + '(' + "name=" + this.dataName + ", displayName=" + this.displayName + ", equation=" + this.equation.asString(
        true) + ", filter=" + (this.filter == null ? "null" : this.filter.asString(true)) + ')';
  }

...
public class AuspiceDataCenter extends AurikaDataCenter {
  ...
  public static AuspiceDataCenter createDefault() {
    ...
    if (Boolean.TRUE.equals(
        AuspiceGlobalConfig.DATABASE_USE_DATA_FOLDER.getManager().noDefault().getBoolean(ZeroArrays.STRING))) {
      var0 = Auspice.get().getPath("data");
    }

    return new AuspiceDataCenter(
        id, var10003, var0, AuspiceGlobalConfig.DATABASE_AUTO_SAVE_INTERVAL.getTime(),
        AuspiceGlobalConfig.DATABASE_LOAD_ALL_DATA_ON_STARTUP.getBoolean(), false,
        AuspiceGlobalConfig.DATABASE_SMART_SAVE.getBoolean()
    );
  }

  @Internal
  public AuspiceDataCenter(
      NSedKey id,
  ...
  protected <T extends DataObject> Database<T> constructDatabase0(String var1, String var2, DataHandler<T> dataHandler) {
    ...
    switch (super.databaseType) {
      case JSON:
        if (singular) {
          return new SingularJsonDatabase<>(
              this.dataDirectory.resolve(var1 + ".json"), (SingularDataHandler<T>) dataHandler);
        }

        return new KeyedJsonDatabase<>(this.dataDirectory.resolve(var1), (KeyedDataHandler) dataHandler);
      case YAML:
        if (singular) {
          return new SingularYamlDatabase<>(
              this.dataDirectory.resolve(var1 + ".yml"), (SingularDataHandler<T>) dataHandler);
        }
      ...
      default:
        ...
        if (!var5) {
          Path var11;
          if (super.databaseType == DatabaseType.H2 && this.sqlConnectionProvider instanceof SQLFlatFileConnectionProvider && Files.exists(
              var11 = ((SQLFlatFileConnectionProvider) this.sqlConnectionProvider).getFile(), new LinkOption[0])) {
            try {
        ...
        return singular ? new SingularSQLDatabase<>(
            super.databaseType, var2, (SingularDataHandler<T>) dataHandler,
            this.sqlConnectionProvider
        ) : new KeyedSQLDatabase<>(
            super.databaseType, var2, (KeyedDataHandler) dataHandler, this.sqlConnectionProvider);
    }
  }
  ...
  public static DatabaseType getDefaultDatabaseType() {
    ...
    DatabaseType configType = Arrays.stream(DatabaseType.values()).filter(
        (var1x) -> var1x.name().toLowerCase().equals(var0)).findFirst().orElse(null);
    if (configType == null) {

...
import net.aurika.auspice.text.compiler.placeholders.TokenCursor;
import net.aurika.auspice.text.compiler.placeholders.types.LanguagePathPlaceholder;
import net.aurika.auspice.text.compiler.placeholders.types.MacroPlaceholder;
        ...
        if (next instanceof TextPiece.Color && prev instanceof TextPiece.Color) {
          if (next.equals(prev)) {
            ...
            if (nextIsHex && prevIsHex) {
              this.exception(
                  this.index - 2, "Two non-formatting colors cannot follow each other as it's overridden by the later.",
                  "||||"
              );
            } else {
              if (!prevIsHex && nextIsHex) {
                this.exception(
                    this.index - 2,
                    "A formatting chat cannot follow a non-formatting chat as it's overridden by the later. Consider putting the formatting chat after the non-formatting one.",
                    "||||"
                );
              }
            }
            ...
            if (result != null) {
              if (result.index <= this.index) {
                throw new IllegalStateException(
                    "Less or no characters consumed for token: " + this.index + " -> " + result.index + " using token handler " + handler);
              }
        ...
        switch (this.pointerChar) {
            ...
            if (!this.settings.translatePlaceholders) {
              ...
              if (placeholderParser != null) {
                ...
                try {
                  ...
                  if (this.settings.validate) {
                    this.exception(
                        this.index, throwable.getMessage(),
                        new String(this.chars, this.index, secondPointer - this.index)
                    );
                  }
          ...
          case '§':
            if (!this.settings.colorize) {
              ...
              if (TextFormatCodes.isTextFormatCode((char) (cache = this.chars[this.index + 1]))) {
                this.compile_clearPlain();
                this.pieces.add(
                    new TextPiece.SimpleFormat(Objects.requireNonNull(TextFormatCodes.getByChar((char) (cache | 32)))));
                ...
                if (this.pointerChar == '&' && cache == 35) {
                  if (this.index + 1 >= this.length - 6) {
                    this.exception(
                        this.index, "Unfinished hex chat",
                        new String(this.chars, this.index, this.length - this.index)
                    );
                    break rawChars;
                  }
                  ...
                  while (var17.length() != 6) {
                    if (!TextFormatCodes.isTextFormatCode((char) (cache = this.chars[++this.index]))) {
                  ...
                  while (var17.length() != 6) {
                    ...
                    if (var19) {
                      ...
                      var19 = true;
                      if (!TextFormatCodes.isTextFormatCode((char) cache)) {
                        continue rawChars;
                      }
                  ...
                  switch (hover_indexChar = textCompiler.chars[var10]) {
                    ...
                    case '}':
                      if (var9 == 0) {
                        ...
                        TextCompiler normalTextCompiler = new TextCompiler(
                            copyChars(hover_normalMessage), textCompiler.settings);
                        TextCompiler hoverTextCompiler = new TextCompiler(
                            copyChars(hover_hoverMessage), textCompiler.settings);
                        TextCompiler clickActionCompiler = new TextCompiler(
                            copyChars(hover_clickAction), textCompiler.settings);
                        TextPiece.Hover var31 = new TextPiece.Hover(
                            action, normalTextCompiler.compileToArray(), hoverTextCompiler.compileToArray(),
                            clickActionCompiler.compileToArray()
                        );
    ...
    switch (this.chars[this.index + 1]) {
      case '#':
        if (!this.settings.colorize) {
          ...
          if (hexColorCode == null) {
            ...
            this.exception(
                pointer_2, "Invalid hex chat length. 3 digit and 6 digit formats are supported", hexColorCode);
            return null;
          } else {
            try {
      ...
      case '$':     //Macro or language path
        if (!this.settings.translatePlaceholders) {
          ...
          if ((str_after_pt_2 = this.validate_specialType(
              isLanguagePath ? TextCompiler.SpecialType.LANGUAGE_PATH : TextCompiler.SpecialType.MACRO)) == null) {
            return null;
          } else if (isLanguagePath) {
            return new TextPiece.Variable(new LanguagePathPlaceholder(
                this.getString(pointer_2 - 2, this.index),
                LanguageEntryMessenger.of(str_after_pt_2), null, new ArrayList<>()
            ));
          } else {
            ...
            if (var22 == null) {
              String var24;
              if ((var24 = Strings.findSimilar(
                  str_after_pt_2, StandardKingdomsPlaceholder.getGlobalMacros().keySet())) == null) {
                var24 = "";
              } else {
            ...
            return new TextPiece.Variable(
                new MacroPlaceholder(
                    this.getString(pointer_2 - 2, this.index), str_after_pt_2, null,
                    new ArrayList<>()
                ));
          }
        }
            ...
            if ((str_after_pt_2 = this.validate_specialType(TextCompiler.SpecialType.BACKREF)) == null) {
              ...
              if (!str_after_pt_2.isEmpty()) {
                try {
                  ...
                  this.exception(
                      this.backRefIndex + 1, "Invalid chat accessor index '" + str_after_pt_2.trim() + '\'',
                      str_after_pt_2
                  );
                  return null;
                }
      ...
      case '?':
        ...
        while (var5.hasNext()) {
          ...
          var4.add(Pair.of(ConditionCompiler.compile(str_after_pt_2).evaluate(), compile(var7).pieces()));
        ...
        if (condition_failed) {
          ...
          var4.add(Pair.of(ConditionCompiler.ConstantLogicalOperand.TRUE, compile(str_after_pt_2).pieces()));
        }
  ...
  public String getString(int startIndex, int endIndex) {
    if (startIndex < 0) {
      throw new IndexOutOfBoundsException(
          "String start index is not positive: " + startIndex + " (" + new String(this.chars) + ')');
    } else if (endIndex >= this.chars.length) {
      throw new IndexOutOfBoundsException(
          "String end index is greater than the actual size: " + endIndex + " >= " + this.chars.length + " (" + new String(
              this.chars) + ')');
    } else if (endIndex <= startIndex) {
      throw new IndexOutOfBoundsException(
          "String end index is less than the start index: " + endIndex + " <= " + startIndex + " (" + new String(
              this.chars) + ')');
    } else {

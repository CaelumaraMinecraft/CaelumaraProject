...
class SocketManager(val logger: Logger) : net.aurika.namespace.NSKedRegistry<SocketHandler>(Auspice.get(), "NETWORK_SOCKET_MANAGER") {
    companion object {
        @JvmStatic
    ...
    val socket = object : SocketJsonCommunicator(4343, logger) {
        override fun onReceive(data: JsonElement) {
            ...
            val ns = net.aurika.namespace.NSedKey.fromString(nsElement.getAsString())
...
abstract class SocketHandler(private val ns: net.aurika.namespace.NSedKey, val needsRequestId: Boolean = false) :
    net.aurika.namespace.NamespacedKeyContainer {

...
public class MessageHandler {
    ...
    public static void sendMessage(CommandSender resolver, String string, boolean usePrefix) {
        ...
        if (!Strings.isNullOrEmpty(string)) {
            TextCompiler.compile(string).getSimpleProvider().send(resolver, (new TextPlaceholderProvider()).usePrefix(usePrefix));
        }
    }
                        ...
                        if (var6 == '\\') {
                            ...
                            if ((var7 = Strings.deleteWhitespace(str.substring(var5 + 1, var6))).length() >= 2) {
                                if (var7.charAt(0) == '#') {
                                    String var10;
                                    TextObject var12;
                                    if ((var12 = StandardKingdomsPlaceholder.getMacro(Strings.toLatinLowerCase(var10 = var7.substring(1)), new TextPlaceholderProvider())) != null) {
                                        builder.append(var12.buildPlain(new TextPlaceholderProvider()));
                                    } else {

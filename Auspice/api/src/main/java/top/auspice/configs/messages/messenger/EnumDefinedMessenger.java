...
public interface EnumDefinedMessenger extends DefinedMessenger {
    ...
    default void sendMessage(@NotNull CommandSender messageReceiver, @NotNull MessagePlaceholderProvider textPlaceholderProvider) {
        ...
        if (CommandAdminTrack.isTracking(messageReceiver) && this != AuspiceLang.COMMAND_ADMIN_TRACK_TRACKED && this != AuspiceLang.COMMAND_ADMIN_TRACK_GUI) {
            ...
            var4 = var8;
            AuspiceLang.COMMAND_ADMIN_TRACK_TRACKED.getProvider((Locale) var3).send(messageReceiver, (new MessagePlaceholderProvider()).parse("path", Strings.join(this.getMessageEntry().getPath(), " {$sep}-> {$s}")).raw("file", var5).raw("raw", LanguageManager.getRawMessage(this, (SupportedLanguage) var3)).parse("line", var4));
        }

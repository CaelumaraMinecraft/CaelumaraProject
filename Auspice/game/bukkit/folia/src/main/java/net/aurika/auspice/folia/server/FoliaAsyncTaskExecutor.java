...
import net.aurika.auspice.server.scheduler.Task;
import net.aurika.auspice.server.scheduler.TaskExecutor;
...
public final class FoliaAsyncTaskExecutor implements TaskExecutor {
    ...
    public Task runDelayed(@NotNull Runnable runnable, long delay, @Nullable TimeUnit timeUnit) {
        ...
        return new FoliaTask(Bukkit.getAsyncScheduler().runDelayed(this.plugin, (__) -> runnable.run(), millis, TimeUnit.MILLISECONDS));
    }
    ...
    public Task repeat(@NotNull Runnable runnable, long initialDelay, long repeatInterval, @Nullable TimeUnit timeUnit) {
        ...
        long intervalMillis = timeUnit != null ? timeUnit.toMillis(repeatInterval) : TaskExecutor.ticksToMillis(repeatInterval);
        return new FoliaTask(Bukkit.getAsyncScheduler().runAtFixedRate(this.plugin, (__) -> runnable.run(), initialMillis, intervalMillis, TimeUnit.MILLISECONDS));
    }

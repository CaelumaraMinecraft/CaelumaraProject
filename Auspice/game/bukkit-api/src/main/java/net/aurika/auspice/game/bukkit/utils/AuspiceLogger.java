package net.aurika.auspice.game.bukkit.utils;
...
public final class AuspiceLogger {
    ...
    public static void debug(DebugIdent debug, Object message) {
        if (isDebugging()) {
            ...
            Server.get().playerManager().getOnlinePlayers().stream().filter(DefaultAuspicePluginPermissions.DEBUG::hasPermission).filter((var1x) -> debug == null || DebugSettings.getSettings(var1x).isWhitelist() == DebugSettings.getSettings(var1x).getList().contains(debug)).forEach((var1x) -> MessageHandler.sendPlayerPluginMessage(var1x, msg));

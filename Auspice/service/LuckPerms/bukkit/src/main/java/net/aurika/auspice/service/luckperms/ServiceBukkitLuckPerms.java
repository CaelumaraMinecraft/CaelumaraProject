...
public final class ServiceBukkitLuckPerms implements ContextCalculator<Player>, BukkitService {
    private static final ServiceBukkitLuckPerms lpService;

    public ServiceBukkitLuckPerms() {
    }
    ...
    public static CompletableFuture<Boolean> perform(Collection<UUID> players, Operation operation, Collection<Node> nodes) {
        ...
        if (luckPerms == null) {
            ...
            return CompletableFuture.allOf(booleanFutures.toArray(new CompletableFuture[0])).thenApply((var0x) -> Boolean.TRUE);
        }
    }
    ...
    public static Node buildNode(ConfigSection section) {
        ...
        {
            NodeBuilder<?, ?> nodeBuilder = Node.builder(Objects.requireNonNull(section.getString(new String[]{"permission"}), () -> "Permission is null " + section));
    ...
    static {
        LuckPerms luckPerms = getLPService();
        ServiceBukkitLuckPerms serviceLP = null;
        if (luckPerms != null) {
            serviceLP = new ServiceBukkitLuckPerms();

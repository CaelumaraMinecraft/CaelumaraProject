package net.aurika.auspice.server.bukkit.permission.registry;

import kotlin.NotImplementedError;
import kotlin.jvm.internal.Intrinsics;
import net.aurika.auspice.server.bukkit.permission.BukkitPermission;
...
//        return permission instanceof BukkitPermission ? (BukkitPermission)permission : new BukkitPermission(new org.bukkit.permissionsPermission(permission.toFullName(), permission.getDescription().parse(new Object[0]), (PermissionDefault)BukkitPermission.DEFAULT_VALUE_MAPPING.get(permission.getDefault())));
    return permission instanceof BukkitPermission ? (BukkitPermission) permission : new BukkitPermission(
        new org.bukkit.permissions.Permission(
            permission.toFullName(),
            BukkitPermission.DEFAULT_VALUE_MAPPING.get(permission.defaultValue())
        ));
  }

...
public final class GUIAccessor {
  ...
  public static @Nullable InteractiveGUI prepare(Player var0, String var1, boolean var2, TextPlaceholderProvider var3, boolean var4) {
    ...
    if ((var7 = GUIConfig.getGUI(var1, var5, var2)) == null) {
      ...
      if (!var7.hasFormBuilder() || !Platform.BEDROCK.isAvailable() || !FloodgateHandler.isFloodgatePlayer(
          var0.getUniqueId())) {
        KLogger.debug(
            KingdomsDebug.BEDROCK$FLOODGATE, () -> {
              return "Inventory GUI Only: hasFormBuilder=" + var7.hasFormBuilder() + ", Bedrock=" + Platform.BEDROCK.isAvailable() + ", isFloodgatePlayer=" + (Platform.BEDROCK.isAvailable() ? FloodgateHandler.isFloodgatePlayer(
                  var0.getUniqueId()) : "no");
            }
        );
        var4 = true;
      ...
      if (CommandAdminTrack.isTracking(var0)) {
        ...
        KingdomsLang.COMMAND_ADMIN_TRACK_GUI.sendMessage(
            var0, new Object[]{"path", var6.replace("/", "{$sep}/{$s}"), "file", var8, "sanitized_path", var8});
      }

      KLogger.debug(
          KingdomsDebug.GUIS_PREPARE, () -> {
      ...
      if (var7.isBroken()) {
        var3.raw("gui_path", var7.getConfig().getFile().toString()).raw(
            "error", var7.getMessage().build(new TextPlaceholderProvider()));
        var7 = a;
        Object var11;
        if (var4) {
        ...
        if (!var9.isAdmin()) {
          if (var7.isCreativeDisallowed() && var0.getGameMode() == GameMode.CREATIVE && !KingdomsDefaultPluginPermission.GUIS_BYPASS_CREATIVE.hasPermission(
              var0, false)) {
          ...
          Messenger var10;
          if (var7.getOpenConditions() != null && (var10 = (Messenger) var7.getOpenConditions().evaluate(
              var3, true)) != null) {
        ...
        return (InteractiveGUI) (var4 ? new InventoryInteractiveGUI(
            var7, var0, var7.getConfig().getConfig(), var3) : new FormInteractiveGUI(
            var7, var0, var7.getConfig().getConfig(), var3));
      }
    }
  }

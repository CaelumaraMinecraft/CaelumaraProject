...
public class KeyedMongoDBDatabase<K, T extends KeyedDataObject<K>> extends MongoDBDatabase<T> implements KeyedDatabase<K, T> {
  ...
  private @NotNull MongoIdQueryContainer<K> a(@NotNull K dataKey) {
    return new MongoIdQueryContainer<>(dataKey, this.getDataHandler().getIdHandler().getKlass());
  }

  private static @NotNull Document __key_doc(@NotNull Object var0) {
    return new Document(PRIMARY_KEY_ID, var0);
  }

  public @Nullable T load(@NotNull K key) {
    Objects.requireNonNull(key, "key");
    Document var10000 = this.getCollection().find(this.a(key)).first();
    if (var10000 == null) {
      return null;
    } else {
      MongoDataProvider var3 = new MongoDataProvider(null, var10000);
      return this.getDataHandler().load(var3, key);
    }
  }

  private K c(Object obj) {
    if (obj instanceof Document document) {
      Codec<K> var10000 = this.getCollection().getCodecRegistry().get(
          this.getDataHandler().getIdHandler().getKlass());  // todo
      Objects.requireNonNull(var10000);
      K var3 = var10000.decode(
          new BsonDocumentReader(document.toBsonDocument()), MongoDBDatabase.DEFAULT_DECODER_CONTEXT);
  ...
  public void save(@NotNull T data) {
    ...
    K var10000 = data.dataKey();
    Objects.requireNonNull(var10000, "");
    Document var3 = __key_doc(var10000);
    MongoDataProvider var4 = new MongoDataProvider(null, var3);
  ...
  public @NotNull Collection<T> loadAllData(@Nullable Predicate<K> keyFilter) {
    ...
    while (true) {
      ...
      try {
        ...
        AuspiceLogger.error(
            "Error while loading '" + var7 + "' of type " + this.getDataHandler().getClass().getSimpleName() + " (Skipping):");
  ...
  public void save(@NotNull Collection<T> data) {
    ...
    if (!data.isEmpty()) {
      ...
      for (T t : data) {
        K key = t.dataKey();
        Objects.requireNonNull(key, "key");
        MongoIdQueryContainer<K> var5 = this.a(key);
        Document var6 = __key_doc(key);
      ...
      this.getCollection().bulkWrite(
          var2, (new BulkWriteOptions()).ordered(false).comment("Save batch data of " + data.size()));
    }
  }

...
public class KeyedSQLDatabase<K, T extends KeyedDataObject.Impl<K>> extends SQLDatabase<T> implements KeyedDatabase<K, T> {
  ...
  public final @Nullable T load(@NotNull K var1) {
    Intrinsics.checkNotNullParameter(var1, "");
    Objects.requireNonNull(var1);
    String var2 = "SELECT * FROM `" + this.getTable() + "` WHERE " + this.getDataHandler().getIdHandler().getWhereClause();
    KeyedSQLDatabase<K, T> var3 = this;
    var2 = var3.handleQuery(var2);

    try (Connection connection = var3.getConnection()) {

      T var46;
      try (PreparedStatement var43 = connection.prepareStatement(var2)) {

        this.getDataHandler().getIdHandler().setSQL(
            new RawSimplePreparedStatement(this.getDatabaseType(), var43), var1);
        ...
        label182:
        {
          label181:
          {
            ...
            try (ResultSet var10 = var44) {
              ...
              SQLDataGetterProvider var47 = new SQLDataGetterProvider(
                  var10002, var10003, null, false, false, new SimpleResultSetQuery(var10009, var10));
              var48 = this.getDataHandler().load(var47, var1);
            }
      ...
      throw new RuntimeException(
          "Error while handling data with query: " + var2 + " with " + SQLDatabase.access$getConnectionProvider(
              var3).getMetaString(), var41
      );
    }
  }
  ...
  public void load(@NotNull Collection<K> keys, @NotNull Consumer<T> dataConsumer) {
    ...
    if (!keys.isEmpty()) {
      ...
      String var4 = Strings.repeat("(" + this.getDataHandler().getIdHandler().getInClause() + "),", keys.size());
      String var5 = var4;
      ...
      try {
        try (Connection var6 = var50.getConnection()) {
          ...
          while (var45.hasNext()) {
            ...
            K k = var45.next();
            this.getDataHandler().getIdHandler().setSQL(
                new RawSimplePreparedStatement(var12 * var3 + 1, this.getDatabaseType(), var51), k);
          }
          ...
          while (var46.next()) {
            ...
            SQLDataGetterProvider var52 = new SQLDataGetterProvider(
                this.getDatabaseType(), this.getTable(), null, false, false,
                new SimpleResultSetQuery(this.getDatabaseType(), var46)
            );
        ...
        throw new RuntimeException(
            "Error while handling data with query: " + var5 + " with " + var50.getConnectionProvider().getMetaString(),
            var44
        );
      }
    }
    ...
    try {
      ...
      try {
        ...
        try {
          var20 = true;
          this.getDataHandler().getIdHandler().setSQL(
              new RawSimplePreparedStatement(this.getDatabaseType(), var26), key);
          var26.execute();
          var20 = false;
      ...
      throw new RuntimeException(
          "Error while handling data with query: " + var2 + " with " + var3.getConnectionProvider().getMetaString(),
          var25
      );
    }
  }
      ...
      try {
        ...
        try {
          ...
          try {
            ...
            while (var44.next()) {
              K var11 = this.getDataHandler().getIdHandler().fromSQL(
                  new SimpleResultSetQuery(this.getDatabaseType(), var44));
              var1.add(var11);
            }
  ...
  public Collection<T> loadAllData(@Nullable Predicate<K> keyFilter) {
    ...
    try {
        ...
        try {
          ...
          try {
            ...
            while (true) {
              ...
              try {
                SQLDataGetterProvider var51 = new SQLDataGetterProvider(
                    this.getDatabaseType(), this.getTable(), null, false, false, var12);
                var2.add(this.getDataHandler().load(var51, var13));
              } catch (Throwable var41) {
                AuspiceLogger.error(
                    "Error while loading '" + var13 + "' of type " + this.getDataHandler().getClass().getSimpleName() + " in table '" + this.getTable() + "' (Skipping):");
      ...
      throw new RuntimeException(
          "Error while handling data with query: " + var3 + " with " + var4.getConnectionProvider().getMetaString(),
          var48
      );
    }
  ...
  public void save(@NotNull @Unmodifiable Collection<T> data) {
    ...
    if (!data.isEmpty()) {
      PreparedNamedSetterStatement var2 = new PreparedNamedSetterStatement(
          this.getDatabaseType(), this.getDataHandler().getSqlProperties().getAssociateNamedData());
      ...
      try {
        ...
        try {
          ...
          while (true) {
            ...
            T var6 = var15.next();
            SQLDataSetterProvider var7 = new SQLDataSetterProvider(
                this.getDatabaseType(), this.getTable(), null, false, false, var2);
        ...
        throw new RuntimeException(
            "Error while trying to save batch data with " + this.getConnectionProvider().getMetaString(), var14);
      }
    }
  }

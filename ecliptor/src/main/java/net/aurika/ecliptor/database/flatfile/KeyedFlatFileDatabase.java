...
public abstract class KeyedFlatFileDatabase<K, T extends KeyedDataObject<K>> implements KeyedDatabase<K, T> {
  ...
  private K a(Path var1) {
    ...
    try {
      ...
      throw new IllegalArgumentException(
          "Malformed data ID from file name: '" + var1 + "' (Did you put/rename a file/folder inside Kingdoms data folders?)");
    }
  }
  ...
  public final void load(@NotNull Collection<K> keys, @NotNull Consumer<T> dataConsumer) {
  ...
  public final void save(@NotNull T data) {
    ...
    String fileName = "" + data.dataKey() + '.' + this.extension;
    Path path = this.folder.resolve(fileName);

    try (BufferedWriter var13 = FSUtil.standardWriter(path)) {
      try (this) {
  ...
  public final Collection<K> getAllDataKeys() {
    ...
    try (DirectoryStream<Path> pathStream = Files.newDirectoryStream(this.folder);) {
  ...
  public final @NotNull Collection<T> loadAllData(@Nullable Predicate<K> keyFilter) {
    ...
    try {
      try (DirectoryStream<Path> var3 = Files.newDirectoryStream(this.folder)) {
          ...
          if (Files.isRegularFile(var6 = o, new LinkOption[0])) {
            ...
            if (keyFilter == null || keyFilter.test(var7)) {
              try {
                ...
                if (var10000 == null) {
                  StringBuilder sb = (new StringBuilder("Couldn't load bulk data ")).append(var7).append(
                      "w -> ").append(var6).append(" -> ");
                  LinkOption[] var10004 = new LinkOption[0];
                  sb.append(Files.exists(var6, Arrays.copyOf(var10004, var10004.length))).append(" -> ").append(
                      this.fileFromKey(var7)).append(" -> ");
                  Path var10003 = this.fileFromKey(var7);
                  var10004 = new LinkOption[0];
                  throw new IllegalStateException(
                      sb.append(Files.exists(var10003, Arrays.copyOf(var10004, var10004.length))).toString());
                }
    ...
    this.d = Math.max(var2.size(), this.d);
    return var2;
  }

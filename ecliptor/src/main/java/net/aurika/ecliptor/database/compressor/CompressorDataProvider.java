package net.aurika.ecliptor.database.compressor;

import net.aurika.ecliptor.api.structured.StructuredDataObject;
import net.aurika.ecliptor.compressor.DataCompressor;
  ...
  public <K, V> void setMap(@NotNull Map<K, ? extends V> value, @NotNull MappingSetterHandler<K, V> handler) {
    ...
    if (!value.isEmpty()) {
      ...
      for (Map.Entry<K, ? extends V> kEntry : value.entrySet()) {
        ...
        handler.map(var4, new StringMappedIdSetter(
            (var100) -> new CompressorDataProvider(var100, CompressorDataProvider.this.getCompressor())), var6
        );
      }
    }
  }

...
public abstract class SQLDatabase<T extends DataObject> implements Database<T> {
  ...
  protected final @NotNull String handleQuery(@NotNull String query) {
    ...
    char var2;
    return (var2 = this.databaseType.systemIdentifierEscapeChar()) != '`' ? StringsKt.replace(
        query, '`', var2, false) : query;
  }
  ...
  public void save(@NotNull T data) {
    ...
    PreparedNamedSetterStatement var3 = new PreparedNamedSetterStatement(
        this.databaseType, dataHandler.getSqlProperties().getAssociateNamedData());
  ...
  protected final <A> A prepareStatement(@NotNull String var1, @NotNull Function<? super PreparedStatement, ? extends A> var2) {
    ...
    try (Connection connection = this.getConnection()) {
      ...
      throw new RuntimeException(
          "Error while handling data with query: " + var1 + " with " + this.getConnectionProvider().getMetaString(),
          var23
      );
    }
  }
  ...
  public final void executeStatement(@NotNull String query) {
    ...
    try (Connection connection = this.getConnection()) {
      ...
      throw new RuntimeException(
          "Error while handling data with query: " + query + " with " + this.connectionProvider.getMetaString(), var23);
    }
  }

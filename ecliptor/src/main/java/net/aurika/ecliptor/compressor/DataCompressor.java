...
import net.aurika.common.key.namespace.Keyed;
...
import net.aurika.ecliptor.api.structured.entries.SimpleMappingDataEntry;
...
public class DataCompressor {
  ...
  public final DataCompressor compress(StructuredDataObject simpleDataObject) {
    return simpleDataObject == null ? compressNull() : compress(simpleDataObject.structuredData());
  }

  public final DataCompressor compress(StructuredData simpleData) {
    if (simpleData == null) {
      return this.compressNull();
    } else {
      DataCompressor compressor = this;
      for (SimpleMappingDataEntry entry : simpleData) {
        compressor = compressor.compress(entry);
      }
      return compressor;
    }
  }

  private DataCompressor compress(SimpleMappingDataEntry entry) {
    if (entry == null) {
      return this.compressNull();
    } else {
      return switch (entry.type()) {
        case INT -> compress(entry.intValue());
        case LONG -> compress(entry.longValue());
        case FLOAT -> compress(entry.floatValue());
        case DOUBLE -> compress(entry.doubleValue());
        case BOOLEAN -> compress(entry.booleanValue());
        case STRING -> compress(entry.stringValue());
      };
    }
  }

  public final DataCompressor compress(Keyed namespacedKeyContainer) {
  ...
  public final DataCompressor compress(UUID uuid) {
    return uuid == null ? this.compressNull() : this.compress(uuid.getLeastSignificantBits()).compress(
  ...
  public final DataCompressor compress(String string) {
    return string == null ? compressNull() : compress(string.length()).compress(

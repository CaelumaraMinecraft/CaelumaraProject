...
public abstract class AurikaDataCenter extends BaseDataManager {
    ...
    protected final @NotNull Map<Key, DataManager<?>> registry = new NonNullMap<>();
    ...
    public void saveAll(SaveType var1) {
        ...
        for (DataManager<?> dataManager : this.getAllDataManagers().stream().sorted(Comparator.comparing((dataManager) -> dataManager.key().asString())).toList()) {
            ...
            var3.pad("\n  &7| &3" + dataManager.key().asString() + "&8: &9", (var10 == null ? var7 : "&cFailed") + "&7/&9" + var6, " &8(&9" + var14 + "ms&8)");
        }
